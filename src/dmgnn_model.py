...
class RelationAwareLayer(MessagePassing):
    ...
    def forward(self, ent_emb, rel_emb, edge_index, edge_type):
        ...
        message_neighbors = self.propagate('add', edge_index, x=ent_emb, edge_type=edge_type, rel_embed=rel_emb,
                                           edge_norm=norm, mode='in')

        # message_self = self.propagate('add', loop_index, x=ent_emb, edge_type=loop_type, rel_embed=rel_emb,
        #                               edge_norm=None, mode='in')
    ...
    def message(self, x_i, x_j, edge_type, rel_embed, edge_norm, mode):
        ...
        score = self.compute_relation_aware_attention(x_i, rel_emb)
        out = torch.mm(rel_emb, weight)  # OK!
...
class EntityAwareLayer(MessagePassing):
    def __init__(self, in_channels, out_channels, rel_dim, act=lambda x: x, args=None):
        super(self.__class__, self).__init__()
    ...
    def message(self, x_i, x_j, edge_type, rel_embed, edge_norm, mode):
        weight = self.gcn_weight
        rel_emb = torch.index_select(rel_embed, 0, edge_type)
        score = self.compute_relation_aware_attention(x_i, x_j)
        out = torch.mm(x_j, weight)  # OK!
        return out, score, edge_norm
...
class CompAwareLayer(MessagePassing):
...
class DMGNN(BaseModel):
    def __init__(self, args, entity_name_emb, num_relations, num_entities):
        ...
        self.atv_mlp    = nn.LeakyReLU(args.leaky_relu_w)
        self.conv1_alignment1 = RelationAwareLayer(self.entity_dim, self.entity_dim, rel_dim=self.entity_dim, act=self.act, args=args)
        self.conv2_alignment1 = RelationAwareLayer(self.entity_dim, self.entity_dim, rel_dim=self.entity_dim, act=self.act, args=args)
        self.conv1_completion1 = RelationAwareLayer(self.entity_dim, self.entity_dim, rel_dim=self.entity_dim, act=self.act, args=args)

        self.conv1_alignment2 = EntityAwareLayer(self.entity_dim, self.entity_dim, rel_dim=self.entity_dim,
                                                  act=self.act, args=args)
        self.conv2_alignment2 = EntityAwareLayer(self.entity_dim, self.entity_dim, rel_dim=self.entity_dim,
                                                  act=self.act, args=args)
        self.conv1_completion2 = EntityAwareLayer(self.entity_dim, self.entity_dim, rel_dim=self.entity_dim,
                                                   act=self.act, args=args)

        self.conv1_alignment3 = CompAwareLayer(self.entity_dim, self.entity_dim, rel_dim=self.entity_dim,
                                                  act=self.act, args=args)
        self.conv2_alignment3 = CompAwareLayer(self.entity_dim, self.entity_dim, rel_dim=self.entity_dim,
                                                  act=self.act, args=args)
        self.conv1_completion3 = CompAwareLayer(self.entity_dim, self.entity_dim, rel_dim=self.entity_dim,
                                                   act=self.act, args=args)
    ...
    def forward_name(self, edge_index, edge_type, ent_bases, rel_bases):
        ...
        ent_align_output1 = self.conv1_alignment1(init_ent_align_att, self.rel_init_att_alignment[rel_bases[0]: rel_bases[1]], edge_index, edge_type)
        ent_align_output2 = self.conv1_alignment2(init_ent_align_att, self.rel_init_att_alignment[rel_bases[0]: rel_bases[1]], edge_index, edge_type)
        ent_align_output3 = self.conv1_alignment3(init_ent_align_att, self.rel_init_att_alignment[rel_bases[0]: rel_bases[1]], edge_index, edge_type)
        # ent_align_output1 = 0.25*ent_align_output1 + 0.25*ent_align_output2 + 0.5*ent_align_output3
        # matrix = torch.stack([ent_align_output1, ent_align_output2, ent_align_output3])
        # ent_align_output1 = torch.min(matrix, dim=0).values

        # ent_align_output1 =  0.25*ent_align_output2 + 0.5*ent_align_output3
        ent_align_output1 = ent_align_output1+ent_align_output2+ent_align_output3
        ...
        if self.args.num_gcn_layer == 2:
            ent_completion_output1 = self.conv1_completion1(init_ent_completion_att, self.rel_init_att_completion[rel_bases[0]: rel_bases[1]], edge_index, edge_type)
            ent_completion_output2 = self.conv1_completion2(init_ent_completion_att, self.rel_init_att_completion[rel_bases[0]: rel_bases[1]], edge_index, edge_type)
            ent_completion_output3 = self.conv1_completion3(init_ent_completion_att, self.rel_init_att_completion[rel_bases[0]: rel_bases[1]], edge_index, edge_type)
            # ent_completion_output1 = 0.25*ent_completion_output1 + 0.25*ent_completion_output2 + 0.5*ent_completion_output3
            # matrix = torch.stack([ent_completion_output1, ent_completion_output2, ent_completion_output3])
            # ent_completion_output1 = torch.min(matrix, dim=0).values
            # ent_completion_output1 = 0.25*ent_completion_output2 + 0.5*ent_completion_output3
            ent_completion_output1 = ent_completion_output1+ent_completion_output2+ent_completion_output3
            ...
            ent_align_output21 = self.conv2_alignment1(ent_in_align_emb, align_rel_input, edge_index, edge_type)
            ent_align_output22 = self.conv2_alignment2(ent_in_align_emb, align_rel_input, edge_index, edge_type)
            ent_align_output23 = self.conv2_alignment3(ent_in_align_emb, align_rel_input, edge_index, edge_type)
            # ent_align_output2 = 0.25*ent_align_output21+ 0.25*ent_align_output22+0.5*ent_align_output23
            # matrix = torch.stack([ent_align_output21, ent_align_output22, ent_align_output23])
            # 对矩阵的每一列进行最大池化
            # ent_align_output2 = torch.min(matrix, dim=0).values
            # ent_align_output2 = 0.25*ent_align_output22+0.5*ent_align_output23
            ent_align_output2 = ent_align_output21+ent_align_output22+ent_align_output23
    ...
    def forward_no_name(self, edge_index, edge_type, ent_bases, rel_bases):
        ...
        if self.args.num_gcn_layer == 2:
            ...
            ent_completion_output1 = self.conv1_completion1(init_ent_completion_att,
                                                            self.rel_init_att_completion[rel_bases[0]: rel_bases[1]],
                                                            edge_index, edge_type)
            ent_completion_output2 = self.conv1_completion2(init_ent_completion_att,
                                                            self.rel_init_att_completion[rel_bases[0]: rel_bases[1]],
                                                            edge_index, edge_type)
            ent_completion_output3 = self.conv1_completion3(init_ent_completion_att,
                                                            self.rel_init_att_completion[rel_bases[0]: rel_bases[1]],
                                                            edge_index, edge_type)
            # ent_completion_output1 = 0.25*ent_completion_output1 + 0.25*ent_completion_output2 + 0.5*ent_completion_output3
            # matrix = torch.stack([ent_completion_output1, ent_completion_output2, ent_completion_output3])
